...
final class FeedUIIntegrationTests: XCTestCase {
  ...
  func test_loadFeedActions_requestFeedFromLoader() {
    ...
    XCTAssertEqual(
      loader.loadFeedCallCount, 0, "Expected no loading requests before view is loaded")
    ...
    XCTAssertEqual(
      loader.loadFeedCallCount, 2, "Expected another loading request once user initiates a reload")
  ...
  func test_loadingFeedIndicator_isVisibleWhileLoadingFeed() {
    ...
    sut.simulateUserInitiatedFeedReload()
    XCTAssertTrue(
      sut.isShowingLoadingIndicator, "Expected loading indicator once user initiates a reload")
  ...
  func test_feedImageView_loadsImageURLWhenVisible() {
    ...
    XCTAssertEqual(
      loader.loadedImageURLs, [], "Expected no image URL requests until views become visible")
  ...
  func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
    ...
    XCTAssertEqual(
      view1?.isShowingImageLoadingIndicator, true,
      "Expected loading indicator state change for second view on retry action")
  }
  ...
  func test_feedImageView_rendersImageLoadedFromURL() {
    ...
    let view1 = sut.simulateFeedImageViewVisible(at: 1)
    XCTAssertEqual(
      view0?.renderedImage, .none, "Expected no image for first view while loading first image")
    XCTAssertEqual(
      view1?.renderedImage, .none, "Expected no image for second view while loading second image")
  ...
  func test_feedImageViewRetryButton_isVisibleOnImageURLLoadError() {
    ...
    XCTAssertEqual(
      view0?.isShowingRetryAction, false,
      "Expected no retry action state change for first view on  second image retry")
    XCTAssertEqual(
      view1?.isShowingRetryAction, false, "Expected no retry action for second view on retry")
  }
  ...
  func test_feedImageViewRetryButton_isVisibleOnInvalidImageData() {
    ...
    XCTAssertEqual(
      view?.isShowingRetryAction, false, "Expected no retry action while loading image")
  ...
  func test_feedImageView_preloadsImageURLWhenNearVisible() {
    ...
    loader.completeFeedLoading(with: [image0, image1])
    XCTAssertEqual(
      loader.loadedImageURLs, [], "Expected no image URL requests until image is near visible")
  ...
  private func makeSUT(file: StaticString = #file, line: UInt = #line) -> (
    ...
    let sut = FeedUIComposer.feedComposedWith(
      feedLoader: loader.loadPublisher, imageLoader: loader.loadImageDataPublisher)
