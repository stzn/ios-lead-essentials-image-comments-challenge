    ...
    func display(_ stubs: [ImageStub]) {
        let cells: [CellController] = stubs.map { stub in
            let cellController = FeedImageCellController(
                viewModel: stub.viewModel, delegate: stub, selection: { })
            stub.controller = cellController
            return CellController(id: UUID(), dataSoruce: cellController, delegate: cellController, prefetchDataSource: cellController)
        }
...
private class ImageStub: FeedImageCellControllerDelegate {
    ...
    func didRequestImage() {
        ...
        guard let image = image else {
            controller?.display(.init(message: "no image"))
            return
        }
