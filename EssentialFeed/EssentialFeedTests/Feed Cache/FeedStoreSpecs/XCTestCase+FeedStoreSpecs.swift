...
extension FeedStoreSpecs where Self: XCTestCase {
  ...
  func assertThatRetrieveDeliversFoundValuesOnNonEmptyCache(
    ...
    expect(
      sut, toRetrieve: .success(CachedFeed(feed: feed, timestamp: timestamp)), file: file,
      line: line)
  }
  ...
  func assertThatRetrieveHasNoSideEffectsOnNonEmptyCache(
    ...
    expect(
      sut, toRetrieveTwice: .success(CachedFeed(feed: feed, timestamp: timestamp)), file: file,
      line: line)
  }
  ...
  func assertThatInsertOverridesPreviouslyInsertedCacheValues(
    ...
    expect(
      sut, toRetrieve: .success(CachedFeed(feed: latestFeed, timestamp: latestTimestamp)),
      file: file, line: line)
  }
  ...
  func assertThatDeleteDeliversNoErrorOnNonEmptyCache(
    ...
    XCTAssertNil(
      deletionError, "Expected non-empty cache deletion to succeed", file: file, line: line)
  }
...
extension FeedStoreSpecs where Self: XCTestCase {
  ...
  func expect(
    _ sut: FeedStore, toRetrieveTwice expectedResult: FeedStore.RetrievalResult,
  ...
  func expect(
    _ sut: FeedStore, toRetrieve expectedResult: FeedStore.RetrievalResult,
    file: StaticString = #file, line: UInt = #line
  ) {
    ...
    sut.retrieve { retrievedResult in
      ...
      default:
        XCTFail(
          "Expected to retrieve \(expectedResult), got \(retrievedResult) instead", file: file,
          line: line)
      }
